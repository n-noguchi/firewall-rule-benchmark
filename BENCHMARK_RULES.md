...
### 1.1 必須システム構成：マスタデータベース

ユーザーが管理する以下のマスタデータは、何らかのデータベースに永続化することを必須とする。
...
このデータベースを、本仕様では`マスタDB`と呼ぶ。

評価システムは、READY状態へ移行する前およびマスタ更新を反映する際に、マスタDBからマスタデータを取得しなければならない。

### 1.1.1 マスタDBとデータ処理の分離

マスタDBを稼働させるDBプロセスと、アクセスログの評価、ルールマッチング、評価用索引の構築などを行うデータ処理プロセスは、別のOSプロセスとして動作し、かつ別のDockerコンテナに配置しなければならない。

少なくとも、マスタDBを提供するDBコンテナと、バッチ評価または逐次評価を実行するアプリケーションコンテナをそれぞれ独立して構成すること。同一コンテナ内でDBとデータ処理を別プロセスとして起動する構成も、本要件を満たさない。

アプリケーションコンテナは、DBコンテナが公開するデータベース接続インターフェースを介してマスタデータを取得・更新しなければならない。アプリケーションプロセスがマスタDBの永続ファイルを直接マウントして読み書きする構成は認めない。マスタDBの永続ファイルはDBコンテナが管理すること。

データベース製品は自由とし、RDBMS、ドキュメントDB、KVSなどを利用できる。組み込みデータベースを利用する場合は、DB専用コンテナ内の独立したDBサービスプロセスとして稼働させ、アプリケーションコンテナから接続可能な管理・取得インターフェースを提供すること。データ処理プロセスが組み込みデータベースライブラリを直接ロードし、同一プロセス内でマスタDBを開く構成は認めない。

マスタDB上の物理スキーマおよび永続化時の論理表現は自由とする。主催者が提供するデータと同じテーブル名、カラム名、正規化単位、レコード粒度で保持する必要はない。ただし、本仕様で定義するデータの意味、識別子、制約および評価結果は維持すること。

正規化、非正規化、複数テーブルへの分割、単一テーブルへの統合、文書形式、Key-Value形式、独自の永続化レイアウトなどを利用してよい。提供データをマスタDBへ投入する際に、参加システム固有のスキーマへ変換してよい。

ただし、採用したスキーマや格納形式にかかわらず、15節で定義する個別の作成・更新・削除を、マスタメンテナンスアプリケーションまたは文書化された管理用インターフェースから容易に実行できること。

アクセスごとにマスタDBを問い合わせる必要はない。マスタDBから取得したデータをもとに、オンメモリ索引、キャッシュ、検索用DB、独自バイナリなどの高速検索用データを構築して評価してよい。

主催者が提供するCSVファイルは、マスタDBへの初期投入または適合試験用の入力データとする。CSVファイルだけをマスタデータの正本として扱い、CSVから評価用データを直接ロードするだけの構成は、本要件を満たさない。
...
マスタデータの正本を1.1節のマスタDBに置くことを前提として、全ルールをメモリに載せる方式、必要なテナントだけロードする方式、DB上で直接評価する方式など、検索・評価方式は自由とする。

---

# 15. マスタデータの保守性要件

1.1節で定義したマスタDB上のデータは、マスタメンテナンスアプリケーションまたは管理用インターフェースから随時変更できる必要がある。

本要件の意図は、提供CSVを編集して差し替える運用を要求することではない。ユーザーが管理するマスタデータの正本をマスタDBに置き、個別の作成・更新・削除を永続化できるシステム構成を要求するものである。

マスタメンテナンスで必要な操作を容易に実行できる限り、マスタDBのスキーマが提供データのレイアウトと異なっていてもよい。保守操作のために提供データ全体の再生成、全件再投入、またはコンテナイメージの再ビルドを要求する構成は、「容易に変更可能」とはみなさない。
...
CSVファイルの編集、CSVファイルの再生成、コンテナイメージの再ビルドだけを更新手段とする構成は認めない。
...
`policies`、`firewall_rules`、`source_ip_groups`、`source_ip_group_members`は、マスタDBへ初期投入するための提供データである。参加システム固有のスキーマへ変換して投入してよい。参加システムは、投入後にマスタDBから取得した内容を評価処理へ反映してからREADYを返すこと。

`access_logs`は各ベンチマーク部門の処理入力であり、マスタDBへの投入は必須としない。
...
単一コンテナ構成は認めない。少なくとも、1.1.1節で定義したDBコンテナとアプリケーションコンテナを必要とする。
...
1.1節で定義したマスタDBは必須構成要素とし、DB専用コンテナも必須とする。DBコンテナと、バッチ評価または逐次評価を行うアプリケーションコンテナは分離しなければならない。同一コンテナへの同居およびデータ処理プロセス内へのDB組み込みは認めない。
