...
データベース製品と配置方式は自由とする。RDBMS、ドキュメントDB、KVS、組み込みデータベースなどを利用でき、アプリケーションと同一コンテナに配置しても、別コンテナに配置してもよい。
...
1.1節で定義したマスタDBは必須構成要素とする。ただし、DB専用コンテナを必須とはしない。組み込みデータベースなど、アプリケーションコンテナ内で動作する構成も認める。
