...
func usage(writer io.Writer) {
	fmt.Fprintln(writer, "usage: firewall-postgres-indexed <batch|batch-client|validate|serve|health|benchmark|verify-results> [flags]")
}
...
func serveCommand(arguments []string) error {
	...
	dataDirectory := flags.String("data", "", "dataset directory")
	databaseURL := flags.String("database-url", "", "PostgreSQL connection URL for the master DB")
	waitTimeout := flags.Duration("master-wait", 2*time.Minute, "max time to wait for the master DB to become reachable")
	...
	if resolvedURL == "" {
		resolvedURL = strings.TrimSpace(os.Getenv("MASTER_DATABASE_URL"))
	}
	if *dataDirectory == "" || resolvedURL == "" {
		return errors.New("--data and --database-url (or MASTER_DATABASE_URL) are required")
	}
	rootCtx, cancel := context.WithTimeout(context.Background(), *waitTimeout)
	defer cancel()
	if err := waitForMaster(rootCtx, resolvedURL, *waitTimeout); err != nil {
		return err
	}
	storeCtx, storeCancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer storeCancel()
	store, err := openMasterStore(storeCtx, resolvedURL)
	if err != nil {
		return err
	}
	defer store.Close()
	importCtx, importCancel := context.WithTimeout(context.Background(), 2*time.Hour)
	defer importCancel()
	if err := store.ensureImported(importCtx, *dataDirectory); err != nil {
		return err
	}
	engine, err := LoadEngineFromMasterStore(context.Background(), store)
	if err != nil {
		return err
	}
