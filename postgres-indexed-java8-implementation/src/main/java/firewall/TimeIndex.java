...
final class TimeIndex {
    ...
    private final int[] bestBySecond;
    ...
    static TimeIndex empty() {
        return new TimeIndex(new ArrayList<TimeRule>(), new Candidate[0], new int[0]);
    }
    ...
    static final class TimeRule {
        final int start;
        final int end;
        final Candidate candidate;

        TimeRule(int start, int end, Candidate candidate) {
            this.start = start;
            this.end = end;
    ...
    static final class Builder {
        private final List<TimeRule> rules = new ArrayList<TimeRule>();
        ...
        TimeIndex build() {
            ...
            rules.sort(new java.util.Comparator<TimeRule>() {
                @Override
                public int compare(TimeRule a, TimeRule b) {
            ...
            List<TimeRule> deduplicated = new ArrayList<TimeRule>();
            for (TimeRule rule : rules) {
            ...
            if (deduplicated.size() < 64) {
                deduplicated.sort(new java.util.Comparator<TimeRule>() {
                    @Override
                    public int compare(TimeRule a, TimeRule b) {
                        return Integer.compare(a.candidate.priority(), b.candidate.priority());
                    }
            ...
            return new TimeIndex(new ArrayList<TimeRule>(), candidates, bestBySecond);
        }
