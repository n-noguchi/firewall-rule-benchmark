...
func readAccessLogFile(path string) ([]AccessLog, error) {
	...
	if !slicesEqual(header, accessHeader) {
...
func ReadExpected(directory string) (map[string]Result, error) {
	...
	for _, path := range files {
		...
		header, err := reader.Read()
		if err == nil && strings.Join(header, ",") != resultHeader {
			err = fmt.Errorf("%s: invalid expected-result header", path)
		}
		for err == nil {
			...
			if row[0] == "" || row[1] == "" || !actionValid(row[3]) {
				err = fmt.Errorf("%s: malformed expected result", path)
				break
			}
			if _, exists := results[row[0]]; exists {
				err = fmt.Errorf("duplicate expected access_id %q", row[0])
				break
			}
...
func ExpectedFiles(directory string) ([]string, error) {
	files, err := csvFiles(directory)
	if err != nil {
		return nil, err
	}
